...
async function generateTestFiles(answers) {
  ...
  for (const file of files) {
    ...
    let destPath = (file.endsWith('page.js.ejs') ? answers.framework === 'cucumber' ? `${destStepRootPath}/pageobjects/${_path.default.basename(file)}` : `${destSpecRootPath}/pageobjects/${_path.default.basename(file)}` : file.includes('step_definition') ? `${answers.stepDefinitions}` : `${destSpecRootPath}/${_path.default.basename(file)}`).replace(/\.ejs$/, '').replace(/\.js$/, answers.isUsingTypeScript ? '.ts' : '.js');
...
async function getAnswers(yes) {
  return yes ? _constants.QUESTIONNAIRE.reduce((answers, question) => Object.assign(answers, question.when && !question.when(answers) ? {} : {
    [question.name]: typeof question.default !== 'undefined' ? typeof question.default === 'function' ? question.default() : question.default : question.choices && question.choices.length ? question.choices[0].value ? question.choices[0].value : question.choices[0] : {}

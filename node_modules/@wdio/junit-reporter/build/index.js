...
class JunitReporter extends _reporter.default {
  ...
  addCucumberFeatureToBuilder(builder, runner, specFileName, suite) {
    ...
    if (suite.type === 'feature') {
      const feature = builder.testSuite().name(featureName).timestamp(suite.start).time(suite._duration / 1000).property('specId', 0).property(this.suiteTitleLabel, suite.title).property('capabilities', runner.sanitizedCapabilities).property(this.fileNameLabel, filePath);
  ...
  addSuiteToBuilder(builder, runner, specFileName, suite) {
    ...
    const filePath = specFileName.replace(process.cwd(), '.');
    let testSuite = builder.testSuite().name(suiteName).timestamp(suite.start).time(suite._duration / 1000).property('specId', 0).property(this.suiteTitleLabel, suite.title).property('capabilities', runner.sanitizedCapabilities).property(this.fileNameLabel, filePath);
  ...
  buildJunitXml(runner) {
    ...
    if (runner.config.hostname !== undefined && runner.config.hostname.indexOf('browserstack') > -1) {
      const browserstackSanitizedCapabilities = [runner.capabilities.device, runner.capabilities.os, (runner.capabilities.os_version || '').replace(/\./g, '_')].filter(Boolean).map(capability => capability.toLowerCase()).join('.').replace(/ /g, '') || runner.sanitizedCapabilities;
